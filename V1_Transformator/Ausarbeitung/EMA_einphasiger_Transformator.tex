...
\begin{document}
	...
	R_{Fe} = f(f)
	$$
	Etwaige Temperaturschwankungen, welche sich auf den Materialwiderstand $R_{Fe}$ auswirken, sind in Realität ebenfalls zu beachten, hier jedoch ausgeschlossen worden.
	...
	\begin{figure}[H]
		...
		\label{fig: Kurzschluss_I1k-U1}
	\end{figure}
	In der Abbildung~\ref{fig: Kurzschluss_I1k-U1} klar ein linearer Verlauf auf der Primärseite des Transformators zu erkennen. Damit ist der Primärseitige Kurzschlussstrom $I_{1k}$ linear abhängig von der Primärspannung $U_1$.
	$$
	I_{1k} = f(U_{1})
	$$
	
	Kurzschlussstrom $I_{1k}$ lässt sich berechnen mit $R_k$, $Z_k$ und $Z_h$.
	\begin{align*}
		R_k &= R_1 + R_2' 
	\end{align*}
